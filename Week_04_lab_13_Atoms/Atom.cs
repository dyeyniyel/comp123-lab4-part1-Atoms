...
namespace Week_04_lab_13_Atoms
{
    public class Atom
    {
        ...
        //default constructor that is necessary for serialization. 
        public Atom() { }

        //constructor assigning the arguments to the appropriate fields.
        ...
        //class method that takes a string and returns an Atom object
        public static Atom Parse(string line)
        {
            string[] subline = line.Split(' '); //splitting string into an array of substrings based on the space delimiter

            if (subline.Length !=5)
            {
                throw new ArgumentException("Invalid input. Expected format is: 'Name Proton Neutron Weight Symbol'"); //exception when argument does not yield five parts

            }
            //parsing the argument into five parts
            string name = subline[0];
            int proton = int.Parse(subline[1]);
            int neutron = int.Parse(subline[2]);
            double weight  = double.Parse(subline[3]);
            string symbol = subline[4];

            return new Atom(name, proton, neutron, weight, symbol); //returns an atom object

        }

        //string representation for the instances of the class
        public override string ToString()
        {
            return $"Name: {Name} ({Symbol}), Proton: {Proton}, Neutron: {Neutron}, Weight:{Weight} ";
        }


        //code as provided in the lab
        ...
        //method to write files to Json format. serialization
        public static void WriteJson(List<Atom> atoms, string filename) //This void class method takes the list of atoms and a filename
        {
            File.WriteAllText(filename, JsonSerializer.Serialize(atoms, new JsonSerializerOptions { WriteIndented = true })); //added JsonSerializerOptions for indention and clearer view
            Console.WriteLine($"\nAll atom objects have been written in JSON format. Filename is {filename}");
        }
        //method to read JSON file for all objects. deserialization
        public static void ReadJson(string filename)
        {
            TextReader reader = new StreamReader(filename);
            List<Atom> atoms = JsonSerializer.Deserialize<List<Atom>>(reader.ReadToEnd()); //deserialize the file contents to a List<Atom> 
            Console.WriteLine($"\nReading file contents of {filename}:");
            foreach (Atom atom in atoms) //for loop to display all items
            {
                Console.WriteLine(atom);
            }
        }

        //Create static methods that does the following and then call them from your main

        //Display all the items in the collection elements using tostring
        public static void DisplayElements(List<Atom> elements)
        {
            Console.WriteLine("List of Atoms in the collection:");
        ...
        public static void WriteJsonFirstItem(Atom atoms, string filename)
        {
            File.WriteAllText(filename, JsonSerializer.Serialize(atoms, new JsonSerializerOptions { WriteIndented = true }));
            Console.WriteLine($"\nFirst atom object has been written in JSON format. Filename is {filename}");
        }

        //method to read JSON file for first object
        public static void ReadJsonFirstItem(string filename)
        {
            TextReader reader = new StreamReader(filename);
            Atom atoms = JsonSerializer.Deserialize<Atom>(reader.ReadToEnd());
            Console.WriteLine($"\nReading file contents of {filename}:");
            Console.WriteLine(atoms.ToString());
        }
        //Serialize the first item using xml format and save to a suitable file
        public static void WriteXMLFirstItem(Atom atoms, string filename)
        {
            XmlSerializer serializer = new XmlSerializer(typeof(Atom));
            TextWriter writer = new StreamWriter(filename);
            serializer.Serialize(writer, atoms);
            Console.WriteLine($"\nFirst atom object has been written in XML format. Filename is {filename}");
            writer.Close();
        }
        //deserialize and display first item
        public static void ReadXMLFirstItem(string filename)
        {
            XmlSerializer serializer = new XmlSerializer(typeof(Atom));
            TextReader reader = new StreamReader(filename);
            Atom atom = (Atom)serializer.Deserialize(reader);
            Console.WriteLine($"\nReading file contents of {filename}");
            Console.WriteLine(atom.ToString());
            reader.Close();
        }
        //Serialize the entire collection in XML format
        public static void WriteXMLAllItems(List<Atom> atoms, string filename)
        {
            XmlSerializer serializer = new XmlSerializer(typeof(List<Atom>));
            TextWriter writer = new StreamWriter(filename);
            serializer.Serialize(writer, atoms);
            Console.WriteLine($"\nAll atom objects has been written in XML format. Filename is {filename}");
            writer.Close();
        }
        public static void ReadXMLAllItems(string filename)
        {
            XmlSerializer serializer = new XmlSerializer(typeof(List<Atom>));
            TextReader reader = new StreamReader(filename);
            List<Atom> atom = (List<Atom>)serializer.Deserialize(reader);
            Console.WriteLine($"\nReading file contents of {filename}");
            foreach (Atom atoms in atom)
            {
                Console.WriteLine(atoms.ToString());
            }
            reader.Close();
